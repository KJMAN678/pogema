...
from gymnasium import Wrapper, logger
from pydantic import BaseModel

from pogema import GridConfig, pogema_v0
from pogema.grid import Grid
from pogema.wrappers.persistence import AgentState, PersistentWrapper
...
class SvgObject:
    ...
    def render_attributes(attributes):
        result = " ".join(
            [f'{x.replace("_", "-")}="{y}"' for x, y in sorted(attributes.items())]
        )
        return result

    def render(self):
        animations = (
            "\n".join([a.render() for a in self.animations])
            if self.animations
            else None
        )
        if animations:
...
class AnimationMonitor(Wrapper):
    ...
    def step(self, action):
        ...
        multi_agent_terminated = isinstance(terminated, (list, tuple)) and all(
            terminated
        )
        ...
        if (
            ...
            or single_agent_terminated
            or multi_agent_truncated
            or single_agent_truncated
        ):
            ...
            if save_tau:
                if (self._episode_idx + 1) % save_tau or save_tau == 1:
                    if not os.path.exists(self.animation_config.directory):
                        logger.info(
                            f"Creating pogema monitor directory {self.animation_config.directory}",
                        )
                        os.makedirs(self.animation_config.directory, exist_ok=True)

                    path = os.path.join(
                        self.animation_config.directory,
    ...
    def create_animation(self, animation_config=None):
        ...
        agents_colors = {
            index: next(colors) for index in range(self.grid_config.num_agents)
        }

        if anim_cfg.egocentric_idx is not None:
            anim_cfg.egocentric_idx %= self.grid_config.num_agents

        decompressed_history: list[list[AgentState]] = self.env.decompress_history(
            self.history
        )
        ...
        if anim_cfg.egocentric_idx is not None:
            ...
            for agent_idx in range(self.grid_config.num_agents):
                decompressed_history[agent_idx] = decompressed_history[agent_idx][
                    :episode_length
                ]
        else:
        ...
        if anim_cfg.static:
            ...
            decompressed_history = [
                [decompressed_history[idx][-1]]
                for idx in range(len(decompressed_history))
            ]

        gh = GridHolder(
            width=len(grid.obstacles),
            height=len(grid.obstacles[0]),
        ...
        render_width, render_height = (
            gh.height * cfg.scale_size + cfg.scale_size,
            gh.width * cfg.scale_size + cfg.scale_size,
        )
        ...
        if anim_cfg.show_lines:
            grid_lines = self.create_grid_lines(
                gh, anim_cfg, render_width, render_height
            )
        ...
        if anim_cfg.egocentric_idx is not None:
            field_of_view = self.create_field_of_view(
                grid_holder=gh, animation_config=anim_cfg
            )
            if not anim_cfg.static:
                self.animate_obstacles(
                    obstacles=obstacles, grid_holder=gh, animation_config=anim_cfg
                )
        ...
        for i in range(-1, grid_holder.height + 1):
            ...
            grid_lines.append(
                Line(
                    ...
                    stroke_width=cfg.stroke_width // 1.5,
                )
            )
    ...
    def save_animation(
        self,
        name="render.svg",
        animation_config: typing.Optional[AnimationConfig] = None,
    ):
        """
        Saves the animation.
    ...
    def animate_field_of_view(self, view, agent_idx, grid_holder):
        ...
        for state in gh.history[agent_idx]:
            x, y = state.get_xy()
            dr = (
                self.grid_config.obs_radius + 1
            ) * cfg.scale_size - cfg.stroke_width * 2
            cx = cfg.draw_start + y * cfg.scale_size
            cy = -cfg.draw_start + -(gh.width - x - 1) * cfg.scale_size
        ...
        visibility = [
            "visible" if state.is_active() else "hidden"
            for state in gh.history[agent_idx]
        ]
        ...
        view.add_animation(
            self.compressed_anim("visibility", visibility, cfg.time_scale)
        )
    ...
    def animate_agents(self, agents, egocentric_idx, grid_holder):
        ...
        for agent_idx, agent in enumerate(agents):
            ...
            for agent_state in gh.history[agent_idx]:
                ...
                y_path.append(
                    str(-cfg.draw_start + -(gh.width - x - 1) * cfg.scale_size)
                )

                if egocentric_idx is not None:
                    ego_x, ego_y = agent_state.get_xy()
                    if self.check_in_radius(
                        x, y, ego_x, ego_y, self.grid_config.obs_radius
                    ):
                        opacity.append("1.0")
                    else:
            ...
            if opacity:
                agent.add_animation(
                    self.compressed_anim("opacity", opacity, cfg.time_scale)
                )
        ...
        for i in range(gh.height):
            for j in range(gh.width):
                ...
                if gh.obstacles[x][y] != self.grid_config.FREE:
                    obs_settings = {}
                    obs_settings.update(
                        x=cfg.draw_start + i * cfg.scale_size - cfg.r,
                    ...
                    if (
                        animation_config.egocentric_idx is not None
                        and cfg.egocentric_shaded
                    ):
                        initial_positions = [
                            agent_states[0].get_xy() for agent_states in gh.history
                        ]
                        ego_x, ego_y = initial_positions[
                            animation_config.egocentric_idx
                        ]
                        if not self.check_in_radius(
                            x, y, ego_x, ego_y, self.grid_config.obs_radius
                        ):
    ...
    def animate_obstacles(self, obstacles, grid_holder, animation_config):
        ...
        for i in range(gh.height):
            for j in range(gh.width):
                ...
                opacity = []
                seen = set()
                for step_idx, agent_state in enumerate(
                    gh.history[animation_config.egocentric_idx]
                ):
                    ego_x, ego_y = agent_state.get_xy()
                    if self.check_in_radius(
                        x, y, ego_x, ego_y, self.grid_config.obs_radius
    ...
    def create_targets(self, grid_holder, animation_config):
        ...
        for agent_idx, agent_states in enumerate(gh.history):
            ...
            if not any(
                [agent_state.is_active() for agent_state in gh.history[agent_idx]]
            ):
                continue

            circle_settings = {}
            circle_settings.update(
                cx=cfg.draw_start + x * cfg.scale_size,
...
def main():
    ...
    env.save_animation(
        "out-static.svg", AnimationConfig(static=True, save_every_idx_episode=None)
    )
    env.save_animation(
        "out-static-ego.svg", AnimationConfig(egocentric_idx=0, static=True)
    )
    env.save_animation(
        "out-static-no-agents.svg", AnimationConfig(show_agents=False, static=True)
    )
