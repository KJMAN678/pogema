...
class PogemaParallel:
    ...
    def __init__(self, grid_config: GridConfig, render_mode="ansi"):
        ...
        self.possible_agents = [
            "player_" + str(r) for r in range(self.pogema.get_num_agents())
        ]
    ...
    def reset(self, seed=None, options=None):
        ...
        self.num_moves = 0
        observations = {
            agent: observations[self.agent_name_mapping[agent]].astype(np.float32)
    ...
    def step(self, actions):
        ...
        actions = [
            actions[agent] if agent in actions else 0 for agent in self.possible_agents
        ]
        observations, rewards, terminated, truncated, infos = self.pogema.step(actions)
        d_observations = {
            agent: observations[anm[agent]].astype(np.float32) for agent in self.agents
        }
        ...
        for agent, idx in anm.items():
            if (
                not self.pogema.grid.is_active[idx] or all(truncated) or all(terminated)
