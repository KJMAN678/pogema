...
def check_grid(obstacles, agents_xy, targets_xy):
    if bool(agents_xy) != bool(targets_xy):
        raise AgentsTargetsSizeError(
            "Agents and targets must be defined together/undefined together!"
        )
    ...
    for i in range(len(agents_xy)):
        for j in range(i + 1, len(agents_xy)):
            if agents_xy[i] == agents_xy[j]:
                raise ValueError(
                    f"Agents can't overlap! {agents_xy[i]} is in both {i} and {j} position."
                )
...
def render_grid(
    obstacles, positions_xy=None, targets_xy=None, is_active=None, mode="human"
):
    ...
    positions_map = {
        (x, y): id_ for id_, (x, y) in enumerate(positions_xy) if is_active[id_]
    }
    finishes_map = {
        (x, y): id_ for id_, (x, y) in enumerate(targets_xy) if is_active[id_]
    }
    for line_index, line in enumerate(obstacles):
        ...
        for cell_index, cell in enumerate(line):
            if cell == CommonSettings().FREE:
                ...
                out += str(
                    gym_utils.colorize(str("   "), "cyan", bold=False, highlight=True)
                )
        out += "\n"
        outfile.write(out)
