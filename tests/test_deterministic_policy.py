...
class DeterministicPolicy:
    def __init__(self, random_seed=42, random_rate=0.2):
        self.agents = None
        self.actions = {
            tuple(GridConfig().MOVES[i]): i for i in range(len(GridConfig().MOVES))
    ...
    def act(self, obs) -> list:
        if self.agents is None:
            ...
            self.agents = [
                AStar() for _ in range(len(obs))
            ]  # create a planner for each of the agents
        actions = []
        for k in range(len(obs)):
            ...
            if (
                start == goal
            ):  # don't waste time on the agents that have already reached their goals
                actions.append(
                    0
                )  # just add useless action to save the order and length of the actions
                continue
            self.agents[k].update_obstacles(obs[k][0], obs[k][1])
            self.agents[k].update_path(start, goal)
            next_node = self.agents[k].get_next_node()
            actions.append(
                self.actions[(next_node[0] - start[0], next_node[1] - start[1])]
            )
