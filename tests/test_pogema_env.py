...
from pogema import (Easy8x8, Easy16x16, Easy32x32, Easy64x64, ExtraHard8x8,
                    ExtraHard16x16, ExtraHard32x32, ExtraHard64x64, Hard8x8,
                    Hard16x16, Hard32x32, Hard64x64, Normal8x8, Normal16x16,
                    Normal32x32, Normal64x64, pogema_v0)
...
def test_moving():
    env = pogema_v0(
        GridConfig(num_agents=2, size=6, obs_radius=2, density=0.3, seed=42)
    )
    ac = ActionMapping()
    env.reset()
...
def test_metrics():
    *_, infos = run_episode(
        GridConfig(num_agents=2, seed=5, size=5, max_episode_steps=64)
    )[-1]
    assert np.isclose(infos[0]["metrics"]["CSR"], 0.0)
    assert np.isclose(infos[0]["metrics"]["ISR"], 0.5)

    *_, infos = run_episode(
        GridConfig(num_agents=2, seed=5, size=5, max_episode_steps=512)
    )[-1]
    assert np.isclose(infos[0]["metrics"]["CSR"], 1.0)
    assert np.isclose(infos[0]["metrics"]["ISR"], 1.0)

    *_, infos = run_episode(
        GridConfig(num_agents=5, seed=5, size=5, max_episode_steps=64)
    )[-1]
    assert np.isclose(infos[0]["metrics"]["CSR"], 0.0)
...
def test_standard_pogema():
    env = pogema_v0(
        GridConfig(
            num_agents=2, size=6, obs_radius=2, density=0.3, seed=42, on_target="finish"
        )
    )
    env.reset()
    run_episode(env=env)
...
def test_predefined_configurations():
    ...
    for make_grid_config_func in predefined_grids:
        ...
        for difficulty, agent_density in zip(
            ...
            if re.match(rf"^{difficulty}\d+x\d+", make_grid_config_func.__name__):
                assert np.isclose(
                    get_target_density_by_num_agents(
                        gc.size, gc.num_agents, gc.density
                    ),
                    agent_density,
                )
...
def test_persistent_env(num_steps=100):
    ...
    for current_step in range(num_steps):
        ...
        first_run_observations.append(
            state_repr(obs, reward, terminated, truncated, info)
        )
        if all(terminated) or all(truncated):
            break
    ...
    for current_step in range(num_steps):
        actions = action_sampler.sample_actions(dim=env.get_num_agents())
        obs, reward, terminated, truncated, info = env.step(actions)
        second_run_observations.append(
            state_repr(obs, reward, terminated, truncated, info)
        )
        assert np.isclose(
            first_run_observations[current_step], second_run_observations[current_step]
        ).all()
        if all(terminated) or all(truncated):
            break
    ...
    for on_target in ["finish", "nothing", "restart"]:
        for num_agents in [1, 32, 64]:
            for size in [32, 64]:
                ...
                table.append(
                    [on_target, num_agents, size, steps_per_second * gc.num_agents]
                )
    print(
        "\n"
        + tabulate(
            table,
            headers=["on_target", "num_agents", "size", "SPS (individual)"],
            tablefmt="grid",
        )
    )
